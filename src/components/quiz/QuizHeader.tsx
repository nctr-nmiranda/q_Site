      ...
      {answeredCount > 0 && (
        <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 flex items-center gap-2 overflow-x-auto">
          <button
            ...
            disabled={showAnswersLoading}
            className={cn(
              'flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0',
              showAnswers
                ? 'bg-blue-100 text-blue-700'
                : 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-100',
              showAnswersLoading && 'opacity-60 cursor-not-allowed'
            )}
          >
            {showAnswersLoading ? (
              <>
            ...
            <div className="flex items-center gap-2 text-sm whitespace-nowrap">
              ...
              {summaryData.pending === 0 && (
                <span className="font-medium">
                  ...
                  <span className="text-blue-600 font-bold">{summaryData.score}%</span>
                </span>
              )}
            </div>
          )}
